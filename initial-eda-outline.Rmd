...
country_tour<-
  ...
  rename("tourism income (/100,000)" = income, Country=name)
sample_n(country_tour,10)
```
...
sunshine_graph_data %>% #use sunshine for left_join, since it contains less countries than country_tour, so there will be less extra
  ...
  na.omit() %>%
  rename(tour = 'tourism income (/100,000)') %>% #renaming bc the previous name would not allow proper graphing
  filter(tour<1000000) %>% # removing outlier (the US)
  ggplot() + geom_point(aes(x=sun_hours_year, y=tour))
```
