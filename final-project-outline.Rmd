...
mean_growth <-
  ...
  na.omit() %>%
  group_by(climate) %>%
  summarise(avg = mean(GrowthRate))
mean_growth

climate %>% 
  left_join(population) %>%
  na.omit() %>%
  ggplot(aes(x = GrowthRate)) + geom_density(aes(colour = climate))  +
  geom_vline(aes(xintercept=mean_growth$avg[1])) +
  geom_vline(aes(xintercept=mean_growth$avg[2])) +
  ggtitle("Growth Rate Density for Tropical & Moderate Climates") +
